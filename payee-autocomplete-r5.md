# 账户名称（Payee）自动补全与筛选器联动机制分析报告（严格四格矩阵版 v5）

---

## 一、命令面板 × 补全列表：严格四格结果矩阵

### 1.1 变量定义

| 变量 | 说明 |
|-----|------|
| **CB** | 命令面板（CommandBar）打开状态：true/false |
| **PL** | 补全列表（PayeeAutocomplete 下拉建议）打开状态：true/false |
| **Ctrl/Cmd+K** | 全局快捷键触发 |

### 1.2 四格结果矩阵

| | PL = false（补全列表关闭） | PL = true（补全列表打开） |
|---|---------------------------|--------------------------|
| **CB = false<br>（命令面板关闭）** | **格 1**：<br>✅ 命令面板正常打开<br><br>**代码证据**：<br>`CommandBar.tsx:152`<br>`if (modalStack.length > 0) return;`<br>条件不满足 → 执行 `setOpen(true)` | **格 2**：<br>✅ 命令面板打开<br>⚠️ 补全列表因输入框失焦自动关闭<br><br>**代码证据**：<br>`CommandBar.tsx:152`<br>内联补全不在 modalStack 中 → 拦截失败<br>`Autocomplete.tsx:525-542`<br>输入框 `onBlur` 触发补全关闭逻辑 |
| **CB = true<br>（命令面板打开）** | **格 3**：<br>⚠️ 命令面板已打开<br>再次按快捷键无变化<br><br>**代码证据**：<br>`CommandBar.tsx:95`<br>`const [open, setOpen] = useState(false);`<br>`setOpen(true)` 对已 true 状态无效果<br>输入焦点已在命令面板搜索框 | **格 4**：<br>❌ 理论上不可能同时存在<br><br>**代码证据**：<br>`CommandBar.tsx:251-254`<br>命令面板使用 `Command.Dialog` 模态渲染<br>打开时会抢夺输入焦点，补全输入框失焦 → 补全列表自动关闭 |

---

## 二、内联补全 vs 模态补全：modalStack 参与方式差异

### 2.1 两种模式的本质区别

| 对比维度 | 内联补全（Inline） | 模态补全（Modal） |
|---------|-------------------|-----------------|
| **触发方式** | 直接渲染 `<PayeeAutocomplete />` 组件 | `dispatch(pushModal({ name: 'payee-autocomplete' }))` |
| **modalStack 状态** | ❌ 不在 modalStack 中 | ✅ 在 modalStack 中 |
| **data-testid** | `autocomplete` | `payee-autocomplete-modal` |
| **Ctrl/Cmd+K 拦截** | ❌ 不拦截（格 2 行为） | ✅ 拦截（格 3 行为） |
| **典型使用场景** | 筛选器、交易表格编辑、规则编辑 | 批量操作、移动端全屏选择器 |
...
**关键特征**：
- 直接在父组件中渲染，无 `pushModal` 调用
- 补全列表使用 Popover 组件渲染在组件树内
- 不在 Redux 的 modalStack 中
...
**关键特征**：
- 通过 `pushModal` 推入 Redux store 的 modalStack
- 使用 `Modal` 组件包装，z-index 更高
- 在 modalStack 中，会被 CommandBar 的拦截逻辑检测到

### 2.4 对四格矩阵的影响说明

> ⚠️ **重要**：四格矩阵中的 "PL = true"（补全列表打开）默认指**内联模式**（最常见的筛选器场景）。
> 
> 如果是**模态模式**的补全列表打开：
> - `modalStack.length ≥ 1`
> - 此时格 2 的行为会变为格 3：命令面板被拦截
> - 但模态补全通常不与筛选器联动使用，因此四格矩阵默认采用内联模式定义
...
### 4.2 建议项排序逻辑 (`getPayeeSuggestions`)
...
### 5.1 PayeeFilter 组件 (`packages/desktop-client/src/components/filters/PayeeFilter.tsx`)
...
### 6.3 报表页筛选应用链路 (`CustomReport.tsx`)
...
### 6.4 公式查询器筛选应用链路 (`QueryManager.tsx`)
...
| Autocomplete 失焦关闭 | `packages/desktop-client/src/components/autocomplete/Autocomplete.tsx` | 525-542 |
