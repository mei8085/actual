# 账户名称（Payee）自动补全与筛选器联动机制分析报告（严格验证版 v4）
...
## 五、与全局命令面板的交互边界（严格验证版）

### 5.1 核心前提验证

#### 5.1.1 modalStack 的定义

**位置**: `packages/desktop-client/src/modals/modalsSlice.ts:691-699`
...
type Modal =  // 预定义的 50+ 种模态框类型，如:
  | { name: 'import-transactions'; options: {...} }
  | { name: 'payee-autocomplete'; options: {...} }
  | { name: 'category-autocomplete'; options: {...} }
  // ... 其他模态框类型
```

**关键代码证据**：
- `pushModal` reducer (`modalsSlice.ts:717-730`): `state.modalStack = [...state.modalStack, modal];`
- `popModal` reducer (`modalsSlice.ts:735-737`): `state.modalStack = state.modalStack.slice(0, -1);`
- `closeModal` reducer (`modalsSlice.ts:738-740`): `state.modalStack = [];`

**结论**: `modalStack` 是 Redux store 中的数组，**仅包含通过 `pushModal` 推入的模态框**。

#### 5.1.2 CommandBar 的状态管理

**位置**: `packages/desktop-client/src/components/CommandBar.tsx:93-157`

```typescript
export function CommandBar() {
  // 证据1: 使用本地 useState 管理 open 状态，不使用 Redux modalStack
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');
  
  // 证据2: 仅读取 modalStack，不修改它
  ...
  const openEventListener = useCallback(
    (e: KeyboardEvent) => {
      if (e.key === 'k' && (e.metaKey || e.ctrlKey)) {
        e.preventDefault();
        // 关键：检查 modalStack.length > 0 来决定是否打开
  ...
  // 证据4: 渲染时使用 Command.Dialog，不使用 Modal 组件
  return (
    <Command.Dialog
      open={open}
...
**关键代码证据**：
- 代码行 95: `const [open, setOpen] = useState(false);` - 本地状态管理
- 代码行 99: `const { modalStack } = useModalState();` - 仅读取，不修改
- 代码行 152: `if (modalStack.length > 0) return;` - 用 modalStack 作为打开条件
- 代码行 251-254: 使用 `Command.Dialog` 渲染，不使用 `Modal` 组件

**结论**: 
- ✅ **CommandBar 自身不在 modalStack 中**
- ✅ CommandBar 仅将 modalStack 作为打开条件进行检查
- ✅ CommandBar 打开时，`modalStack.length` 保持不变

#### 5.1.3 纠正历史错误

> ❌ 错误结论（r2/r3 版本）："命令面板打开时 modalStack.length > 0"
> 
> ✅ 正确结论：**命令面板打开时，modalStack.length 可能为 0**
> 
> 证据：CommandBar 打开仅设置 `setOpen(true)`，不调用 `pushModal`，不修改 Redux store 中的 modalStack。

### 5.2 PayeeAutocomplete 的两种使用模式

| 模式 | 触发方式 | 是否加入 modalStack | data-testid | 典型使用场景 |
|-----|---------|-------------------|------------|-------------|
| **内联/嵌入式模式** | 直接渲染组件 | ❌ 不加入 | `autocomplete` | 筛选器、交易表格编辑、规则编辑 |
| **模态框模式** | `pushModal({ name: 'payee-autocomplete' })` | ✅ 加入 | `payee-autocomplete-modal` | 批量操作、移动端全屏选择器 |

#### 5.2.1 内联模式（筛选器中）

**位置**: `packages/desktop-client/src/components/filters/PayeeFilter.tsx:47-57`
...
**证据**：
- 直接渲染组件，无 `pushModal` 调用
- 不在 modalStack 中

#### 5.2.2 模态框模式

**位置**: `packages/desktop-client/src/components/modals/PayeeAutocompleteModal.tsx:24-90`
...
export function PayeeAutocompleteModal({
  ...
  return (
    <Modal name="payee-autocomplete" onClose={onClose}>
      {({ state }) => (
        <PayeeAutocomplete
          focused
          embedded
...
**证据**：
- 通过 `pushModal` 推入 Redux store
- 在 modalStack 中

### 5.3 filters popover 是否属于 modalStack 拦截范围？

**结论**: **filters popover（`filters-menu-tooltip`）不属于 modalStack 拦截范围**。

**证据**：

1. **filters popover 实现方式**:
   - 位置: `FiltersMenu.tsx:790-795` 和 `FilterExpression.tsx:147-152`
   - 使用 `@actual-app/components/popover` 组件渲染
   - 不调用 `pushModal`
   - 有 `data-testid="filters-menu-tooltip"` 标识

2. **modalStack 的特殊检查（仅一处）**:
   - 位置: `modalsSlice.ts:717-728`
   ```typescript
   if (
   ...
   ```
   - **仅针对 `keyboard-shortcuts` 模态框**的特殊处理
   - **不影响 CommandBar** 的打开逻辑

3. **CommandBar 不检查 filters popover**:
   - 位置: `CommandBar.tsx:147-157`
   - 仅检查 `modalStack.length > 0`
   - 不查询 DOM 中的 `filters-menu-tooltip` 元素

### 5.4 Ctrl/Cmd+K 键盘事件的精确传播链

#### 5.4.1 Autocomplete 组件的键盘处理

**位置**: `packages/desktop-client/src/components/autocomplete/Autocomplete.tsx:544-612`
...
onKeyDown: (e: KeyboardEvent<HTMLInputElement>) => {
  ...
  if (isOpen) {
    if (e.key === 'Enter') {
      // 处理 Enter 键
      e.stopPropagation();  // 仅阻止 Enter 传播
  ...
  if (e.key === 'Escape') {
    // 处理 Escape 键
    e.stopPropagation();  // 仅阻止 Escape 传播
  }
  
  // 证据: 对 Ctrl/Cmd+K 无任何处理
  // 没有 stopPropagation()
  // 没有 preventDefault()
  // 事件正常冒泡
}
...
**证据**：
- 仅对 `Enter`、`Escape`、`Tab`、方向键做特殊处理
- 对 `Ctrl/Cmd+K` **无任何拦截逻辑**
...
输入框原生 keydown 事件
    ↓
Autocomplete.onKeyDown(e)  [Autocomplete.tsx:544]
    ├─ 检查 e.key 是否为 Enter/Escape/Tab/ArrowUp/ArrowDown
    └─ e.key === 'k'，不匹配任何处理分支
       → 不调用 stopPropagation()
       → 不调用 preventDefault()
       → 事件继续冒泡
        ↓
事件冒泡到父元素（FiltersMenu、FilterExpression 等）
    ↓
父元素无针对 Ctrl/Cmd+K 的特殊处理
    ↓
事件继续冒泡到 document
    ↓
CommandBar.openEventListener(e)  [CommandBar.tsx:147]
    ├─ 检查 e.key === 'k' && (e.metaKey || e.ctrlKey) → 匹配
    ├─ 调用 e.preventDefault() → 阻止浏览器默认行为
    ├─ 检查 modalStack.length > 0 → 根据当前状态决定
    └─ 条件满足时调用 setOpen(true)
```

### 5.5 严格四格结果矩阵（可验证）

**变量定义**:
- **CB**: CommandBar 打开状态（true/false）
- **PL**: Payee 补全列表打开状态（true/false）
- **modalStack.length**: Redux store 中的模态框栈长度

| 场景 | CB 状态 | PL 状态 | modalStack.length | Ctrl/Cmd+K 结果 | 代码证据 |
|-----|--------|--------|------------------|----------------|----------|
| **场景 1** | false | false | = 0 | ✅ CommandBar 正常打开 | `CommandBar.tsx:152` 条件满足，`setOpen(true)` |
| **场景 2** | false | true（内联） | = 0 | ✅ CommandBar 打开<br>⚠️ PL 因失焦关闭 | PL 不在 modalStack → 拦截失败<br>输入框失焦 → PL 自动关闭 |
| **场景 3** | false | true（模态） | ≥ 1 | ❌ CommandBar 不打开 | `CommandBar.tsx:152` 条件不满足<br>直接 return |
| **场景 4** | true | false | = 0 | ⚠️ CommandBar 已打开<br>再次按快捷键无变化 | CB 打开时 modalStack 仍为 0<br>但 `setOpen(true)` 无效果 |
| **场景 5** | true | true（内联） | = 0 | ❌ 理论上不可能<br>CB 打开时输入焦点在 CB 搜索框 | CB 打开时 PL 输入框失焦<br>PL 自动关闭 |
| **场景 6** | true | true（模态） | ≥ 1 | ❌ 不可能同时存在 | CB 打开前检查 `modalStack.length > 0` 会被拦截 |

### 5.6 交互边界的最终结论（可验证）

1. **CommandBar 与 modalStack 的关系**
   - ✅ CommandBar 自身 **不在** modalStack 中
   - ✅ CommandBar 仅读取 modalStack 作为打开条件
   - ✅ CommandBar 打开时，`modalStack.length` 保持不变

2. **PayeeAutocomplete 与 modalStack 的关系**
   - ✅ 内联模式：**不在** modalStack 中
   - ✅ 模态模式：**在** modalStack 中（通过 `pushModal` 加入）

3. **键盘交互结论**
   - ✅ 内联补全列表打开时，Ctrl/Cmd+K 会打开 CommandBar
   - ✅ 模态补全打开时，Ctrl/Cmd+K 被拦截
   - ✅ filters popover 打开时，Ctrl/Cmd+K 不会被拦截（modalStack 仍为 0）

4. **无直接组件依赖**
   - CommandBar 和 PayeeAutocomplete 没有共享状态
   - 没有互相调用的函数
   - 唯一关联是通过 modalStack 间接影响
...
| Autocomplete 键盘处理 | `packages/desktop-client/src/components/autocomplete/Autocomplete.tsx` | 544-612 |
...
5. **状态独立**: CommandBar 不在 modalStack 中，两套状态管理完全独立
